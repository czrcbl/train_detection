...
def unormalize(img, mean=(0.485, 0.456, 0.406), std=(0.229, 0.224, 0.225)):
    """Transforms a normalized ndarray into a numpy image"""
    img = (img * mx.nd.array(std).reshape((3, 1, 1)) + mx.nd.array(mean).reshape((3, 1, 1))) * 255
...
class SSDTrainTransform(object):
    ...
    def __init__(self, width, height, anchors=None, mean=(0.485, 0.456, 0.406),
                 ...
                 bilateral_kernel_size=None, sigma_vals=None, grayscale=False, **kwargs):
        self._width = width
        self._height = height
    ...
    def __call__(self, src, label):
        ...
        if (self._bilateral_kernel_size is not None) and (self._sigma_vals is not None) or self._grayscale:
            img = img.asnumpy()
            if (self._bilateral_kernel_size is not None) and (self._sigma_vals is not None):
                img = cv2.bilateralFilter(img, self._bilateral_kernel_size, self._sigma_vals, self._sigma_vals)
            if self._grayscale:
...
class SSDValTransform(object):
    ...
    def __init__(self, width, height, mean=(0.485, 0.456, 0.406), std=(0.229, 0.224, 0.225),
                 bilateral_kernel_size=None, sigma_vals=None, grayscale=False):
        self._width = width
        self._height = height
        self._mean = mean
        self._std = std
