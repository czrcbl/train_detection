...
def parse_args():
    ...
    parser.add_argument('--transfer-from', default='coco', help='Dataset from which apply transfer learning.')
...
if __name__ == '__main__':
    ...
    net_name = '_'.join(('faster_rcnn', *module_list, args.network, args.transfer_from))
    if args.custom_model:
        args.use_fpn = True
    ...
    if args.resume.strip():
        ...
        for param in net.collect_params().values():
            if param._data is not None:
                continue
            param.initialize()
    net.short = args.short
    net.max_size = args.max_size
    net.collect_params().reset_ctx(ctx)
    print('Net short:', net.short)
